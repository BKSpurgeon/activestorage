...
3. Run `bundle install` in order to install the gem added above.
4. Run `rails activestorage:install` to create needed directories, migrations, and configuration.
5. Configure the storage service in `config/environments/*` with `config.active_storage.service = :local`
   that references the services configured in `config/storage_services.yml`.
6. Optional: Add `gem "aws-sdk", "~> 2"` to your Gemfile if you want to use AWS S3.
7. Optional: Add `gem "google-cloud-storage", "~> 1.3"` to your Gemfile if you want to use Google Cloud Storage.
8. Optional: Add `gem "mini_magick"` to your Gemfile if you want to use variants.
